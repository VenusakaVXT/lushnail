...
// Copy all fonts from src/assets/fonts -> dist/assets/fonts
function copyAllFonts() {
  const srcDir = path.join('src', 'assets', 'fonts');
  ...
  copyDir(srcDir, destDir);
  console.log(`Copied fonts: ${srcDir} -> ${destDir}`);
}
...
function buildAll() {
  ...
  copyAllImages();
  // Copy all fonts once on initial build
  copyAllFonts();
